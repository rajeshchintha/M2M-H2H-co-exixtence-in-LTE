...
NS_LOG_COMPONENT_DEFINE ("CosemLteExample");
...
main (int argc, char *argv[])
{
  ...
  CommandLine cmd;
  cmd.AddValue("numberOfeNodeBs", "Number of eNodeBs + UE pairs", numberOfNodes);
  ...
  lteHelper->SetEpcHelper (epcHelper);//Commented by me
 ...
 Ptr<Node> pgw = epcHelper->GetPgwNode ();//Commented by me
 ...
 //Ipv4Address remoteHostAddr = internetIpIfaces.GetAddress (1);//uncommented by me
  ...
  // Attach one UE per eNodeB  //This was giving error: msg="UEs need to have IPv4 installed before EPS bearers can be activated"  i am shifting entire block after Install IP stack
  /*for (uint16_t i = 0; i < numberOfNodes; i++)
      {
...
// Attach one UE per eNodeB
  ...
  lteHelper->ActivateDedicatedEpsBearer(ueLteDevs, EpsBearer (EpsBearer::NGBR_VIDEO_TCP_DEFAULT), EpcTft::Default ());//changed ActivateEpsBearer to ActivateDedicatedEpsBearer by me


  // Install and start applications on UEs and remote host
  /*uint16_t dlPort = 1234;//original block comment starts here, uncommented by me 
  uint16_t ulPort = 2000;
  uint16_t otherPort = 3000;
  PacketSinkHelper dlPacketSinkHelper ("ns3::UdpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), dlPort));
  PacketSinkHelper ulPacketSinkHelper ("ns3::UdpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), ulPort));
  PacketSinkHelper packetSinkHelper ("ns3::UdpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), otherPort));
  ApplicationContainer clientApps;//commented by me
  ApplicationContainer serverApps;//commented by me
  for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
    {
      ++ulPort;
      ++otherPort;
  ...
  UdpCosemClientHelper cosemClient (serverApps, clientLteInterfaces, Seconds (5.0));
