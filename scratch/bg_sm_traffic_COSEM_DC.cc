/*
 * This file is made by changing bg_sm_traffic.cc  & cosemDcWifiLte.cc by RajeshChintha on 4th April 2017
 * Objective is to add DC lte
 * DC code is taken from cosemDcWifiLte.cc
 ************************************************
 * Successful in communicating with 2 DCs from 2 MDMs and 2 DRs (nCcCsma = 4)-----------6th April 2017
 * Next step is allocating group of SMs to each DC
 */
...
//**************************************************************************************************
	...
	uint16_t numberOfSmNodes = 20;//Number of SMs 
...
//*************************************************************************************************
	...
	CommandLine cmd;
	cmd.AddValue("numberOfSmNodes", "Number of SMs", numberOfSmNodes);
...
for (uint16_t i = 0; i < numberOfSmNodes; i++)
	{
...
//COSEM Servers
  ...
  serverApps.Stop (Seconds (simTime));//10.0));

//COSEM Clients (DCs)
 //DC1
  UdpCosemClientHelper cosemClient (serverApps, dcIpIface, Seconds (reqSmTime));//requestInterval));//5.0));//modified by me
  ApplicationContainer clientApps = cosemClient.Install (dcNodes.Get (0));
  clientApps.Start (Seconds (0.2));
  clientApps.Stop (Seconds (simTime));
//DC2
  ApplicationContainer clientApps2 = cosemClient.Install (dcNodes.Get (1));
...
//DR2
  DemandResponseApplicationHelper drHelper2 (CcCsmaInterfaces.GetAddress (3), mdmApps2);
  ApplicationContainer drApps2 = drHelper2.Install (CcCsmaNodes.Get (3));
