...
/**
 * Sample simulation script for LTE+EPC. It instantiates one eNodeB (Changd by me)(//several eNodeB),
...
//  Ipv4Address remoteHostAddr = internetIpIfaces.GetAddress (1);//uncommented by me
  ...
  enbNodes.Create(1);//changed from numberOfNodes to 1 by me
...
// Attach one UE per eNodeB
  for (uint16_t i = 0; i < numberOfNodes; i++)
      {
        lteHelper->Attach (ueLteDevs.Get(i), enbLteDevs.Get(0));//changed from i to 0 by me
      }
  ...
  UdpCosemClientHelper cosemClient (serverApps, clientLteInterfaces, Seconds (60.0));//5.0));//changed by me
